...
function row(comparison: ReturnType<typeof compareCapacityScenarios>, resourceGroupId: string, periodStart: string) {
...
describe("governed recovery scenarios", () => {
  ...
  it("applies added positioners only from their effective date", () => {
  ...
  it("applies overflow capacity from its approved start", () => {
    const comparison = compareCapacityScenarios(northstarRecoveryModel, "baseline", "recovery-1");
