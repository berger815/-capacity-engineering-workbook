...
export type RoutingImportResult = ImportResult<
  RoutingRevision,
  RoutingImportControlTotals
>;
...
const states: ApplicabilityState[] = [
  "value",
  "missing",
  "notApplicable",
  "zero",
];
const bases: RequirementBasis[] = ["perUnit", "perProgram", "perPeriod"];
...
function resolveProduct(
  ...
  if (!mapping.productExternalKey)
    throw new Error(
      "productExternalKey is required when productMatch=externalKey",
    );
  return products.find(
    (product) =>
      product.externalKeys?.[mapping.productExternalKey ?? ""] === key,
  );
}

function parseState(raw: string): ApplicabilityState | null {
  const normalized = raw.trim();
  return states.includes(normalized as ApplicabilityState)
    ? (normalized as ApplicabilityState)
    : null;
}
...
export function importRoutingCsv(
  ...
  const resourceGroups = new Map(
    model.resourceGroups.map((group) => [group.id, group]),
  );
  ...
  const parsedRows: RawRoutingRow[] = table.rows.map((row, index) => {
    ...
    const revisionName = mapping.revisionColumn
      ? textValue(row, mapping.revisionColumn, mapping)
      : revisionSourceId;
    const revisionId =
      product && revisionSourceId
        ? `${product.id}:${revisionSourceId}`
        : revisionSourceId;
    const effectiveFromRaw = textValue(
      row,
      mapping.effectiveFromColumn,
      mapping,
    );
    ...
    const allocation = allocations.includes(allocationRaw as PhaseAllocation)
      ? (allocationRaw as PhaseAllocation)
      : null;
    const operationSourceId = textValue(
      row,
      mapping.operationIdColumn,
      mapping,
    );
    const operationId =
      revisionId && operationSourceId
        ? `${revisionId}:${operationSourceId}`
        : operationSourceId;
    const operationName = textValue(row, mapping.operationNameColumn, mapping);
    const sequenceRaw = textValue(
      row,
      mapping.operationSequenceColumn,
      mapping,
    );
    const sequence = parseNumber(sequenceRaw);
    const resourceGroupId = textValue(
      row,
      mapping.resourceGroupIdColumn,
      mapping,
    );
    const requirementStateRaw = textValue(
      row,
      mapping.requirementStateColumn,
      mapping,
    );
    const requirementState = parseState(requirementStateRaw);
    const requirementValueRaw = textValue(
      row,
      mapping.requirementValueColumn,
      mapping,
    );
    const parsedRequirementValue = requirementValueRaw
      ? parseNumber(requirementValueRaw)
      : null;
    const basisRaw = textValue(row, mapping.basisColumn, mapping);
    const basis = basisRaw
      ? bases.includes(basisRaw as RequirementBasis)
        ? (basisRaw as RequirementBasis)
        : null
      : "perUnit";
    const setupStateRaw = textValue(
      row,
      mapping.setupRequirementStateColumn,
      mapping,
    );
    const setupState = setupStateRaw ? parseState(setupStateRaw) : null;
    const setupValueRaw = textValue(
      row,
      mapping.setupRequirementValueColumn,
      mapping,
    );
    const setupValue = setupValueRaw ? parseNumber(setupValueRaw) : null;
    const setupQuantityRaw = textValue(
      row,
      mapping.setupQuantityColumn,
      mapping,
    );
    const setupQuantity = setupQuantityRaw
      ? parseNumber(setupQuantityRaw)
      : null;
    ...
    if (!product)
      rowIssues.push({
        ...
        message: `Product '${productRaw}' was not found`,
    ...
    if (!revisionSourceId)
      rowIssues.push({
        ...
        column: mapping.revisionIdColumn,
      });
    if (!revisionName)
      rowIssues.push({
        rowNumber,
        severity: "error",
        code: "REVISION_NAME_REQUIRED",
        message: "Revision name is required",
        column: mapping.revisionColumn ?? mapping.revisionIdColumn,
      });
    if (!effectiveFrom)
      rowIssues.push({
        rowNumber,
        entityKey: revisionId,
        severity: "error",
        code: "REVISION_EFFECTIVE_DATE_INVALID",
        message: `Invalid effectiveFrom '${effectiveFromRaw}'`,
        column: mapping.effectiveFromColumn,
        value: effectiveFromRaw,
      });
    if (effectiveToRaw && !effectiveTo)
      rowIssues.push({
        ...
        severity: "error",
        code: "REVISION_EFFECTIVE_DATE_INVALID",
        message: `Invalid effectiveTo '${effectiveToRaw}'`,
        column: mapping.effectiveToColumn,
        value: effectiveToRaw,
      });
    if (effectiveFrom && effectiveTo && effectiveTo < effectiveFrom)
      rowIssues.push({
        ...
        entityKey: revisionId,
        severity: "error",
        code: "REVISION_EFFECTIVE_RANGE_INVALID",
    ...
    if (!phaseName)
      rowIssues.push({
        ...
        entityKey: phaseId,
        severity: "error",
        code: "PHASE_NAME_REQUIRED",
        message: "Phase name is required",
        column: mapping.phaseNameColumn,
      });
    if (
      startWeeks === null ||
      startWeeks < 0 ||
      ...
      rowIssues.push({
        ...
        entityKey: phaseId,
        severity: "error",
        code: "PHASE_RANGE_INVALID",
        message: `Lead-time phase must have start >= end >= 0; received '${startRaw}' and '${endRaw}'`,
      });
    if (!allocation)
      rowIssues.push({
        rowNumber,
        entityKey: phaseId,
        severity: "error",
        code: "PHASE_ALLOCATION_INVALID",
        message: `Invalid phase allocation '${allocationRaw}'`,
        column: mapping.allocationColumn,
        value: allocationRaw,
      });
    if (!operationSourceId)
      rowIssues.push({
        rowNumber,
        entityKey: revisionId,
        severity: "error",
        ...
        column: mapping.operationIdColumn,
      });
    if (!operationName)
      rowIssues.push({
        rowNumber,
        entityKey: operationId,
        severity: "error",
        code: "OPERATION_NAME_REQUIRED",
        message: "Operation name is required",
        column: mapping.operationNameColumn,
      });
    if (sequence === null || !Number.isInteger(sequence) || sequence < 0)
      rowIssues.push({
        ...
        entityKey: operationId,
        severity: "error",
        code: "OPERATION_SEQUENCE_INVALID",
        message: `Operation sequence must be a nonnegative integer; received '${sequenceRaw}'`,
        column: mapping.operationSequenceColumn,
        value: sequenceRaw,
      });
    ...
    if (!group)
      rowIssues.push({
        ...
        message: `Resource group '${resourceGroupId}' was not found`,
    ...
    if (!requirementState)
      rowIssues.push({
        ...
        message: `Invalid requirement state '${requirementStateRaw}'`,
        column: mapping.requirementStateColumn,
        value: requirementStateRaw,
      });
    if (
      requirementState === "value" &&
      (parsedRequirementValue === null || parsedRequirementValue < 0)
    )
      rowIssues.push({
        rowNumber,
        entityKey: operationId,
        severity: "error",
        code: "REQUIREMENT_VALUE_REQUIRED",
        message: `state=value requires a nonnegative requirement value; received '${requirementValueRaw}'`,
        column: mapping.requirementValueColumn,
        value: requirementValueRaw,
      });
    if (
      requirementState !== "value" &&
      parsedRequirementValue !== null &&
      parsedRequirementValue !== 0
    )
      rowIssues.push({
        rowNumber,
        entityKey: operationId,
        severity: "error",
        code: "REQUIREMENT_VALUE_INVALID",
        message: "Only state=value may carry a non-zero requirement value",
        column: mapping.requirementValueColumn,
        value: requirementValueRaw,
      });
    if (!basis)
      rowIssues.push({
        ...
        entityKey: operationId,
        severity: "error",
        code: "REQUIREMENT_BASIS_INVALID",
        message: `Invalid requirement basis '${basisRaw}'`,
        column: mapping.basisColumn,
        value: basisRaw,
      });
    if (setupStateRaw && !setupState)
      rowIssues.push({
        rowNumber,
        entityKey: operationId,
        severity: "error",
        code: "SETUP_REQUIREMENT_STATE_INVALID",
        message: `Invalid setup requirement state '${setupStateRaw}'`,
        column: mapping.setupRequirementStateColumn,
        value: setupStateRaw,
      });
    if (setupState === "value" && (setupValue === null || setupValue < 0))
      rowIssues.push({
        rowNumber,
        entityKey: operationId,
        severity: "error",
        code: "SETUP_REQUIREMENT_VALUE_REQUIRED",
        message: `setup state=value requires a nonnegative value; received '${setupValueRaw}'`,
        column: mapping.setupRequirementValueColumn,
        value: setupValueRaw,
      });
    if (setupQuantity !== null && setupQuantity < 0)
      rowIssues.push({
        rowNumber,
        entityKey: operationId,
        severity: "error",
        code: "SETUP_QUANTITY_INVALID",
        message: `setupQuantity must be nonnegative; received '${setupQuantityRaw}'`,
        column: mapping.setupQuantityColumn,
        value: setupQuantityRaw,
      });
    if (batchSize !== null && batchSize <= 0)
      rowIssues.push({
        ...
        message: `batchSize must be greater than zero; received '${batchSizeRaw}'`,
        column: mapping.batchSizeColumn,
        value: batchSizeRaw,
      });
  ...
  const revisionGroups = groupBy(
    parsedRows,
    (row) => row.revisionId || `row-${row.rowNumber}`,
  );
  ...
  for (const [revisionKey, revisionRows] of revisionGroups) {
    ...
    if (!first) continue;
    const revisionLevelCodes = new Set([
      "PRODUCT_UNKNOWN",
      "REVISION_ID_REQUIRED",
      "REVISION_NAME_REQUIRED",
      "REVISION_EFFECTIVE_DATE_INVALID",
      "REVISION_EFFECTIVE_RANGE_INVALID",
    ]);
    let revisionRejected = revisionRows.some((row) =>
      row.issues.some((issue) => revisionLevelCodes.has(issue.code)),
    );
    ...
    for (const [label, getter] of consistentFields) {
      ...
      if (values.size > 1) {
        ...
        issues.push({
          entityKey: revisionKey,
          severity: "error",
          code: "REVISION_INCONSISTENT",
          message: `Revision rows disagree on ${label}`,
        });
      }
    }

    if (!revisionRejected && first.product && first.effectiveFrom) {
      const candidates = [...model.routingRevisions, ...records].filter(
        (revision) => revision.productId === first.product?.id,
      );
      if (
        candidates.some((revision) =>
          intervalsOverlap(
            first.effectiveFrom ?? "",
            first.effectiveTo ?? undefined,
        ...
        issues.push({
          ...
          message: `Revision '${first.revisionName}' overlaps another effective-dated revision for product '${first.product.id}'`,
        });
      }
    }
    ...
    if (
      ...
      rejectedOperationCount += new Set(
        revisionRows.map((row) => row.operationId),
      ).size;
      rejectedPhaseCount += new Set(revisionRows.map((row) => row.phaseId))
        .size;
      continue;
    }
    ...
    const phaseGroups = groupBy(
      revisionRows,
      (row) => row.phaseId || `row-${row.rowNumber}`,
    );
    ...
    for (const [phaseKey, phaseRows] of phaseGroups) {
      ...
      const phaseCodes = new Set([
        ...
        "PHASE_NAME_REQUIRED",
        "PHASE_RANGE_INVALID",
        "PHASE_ALLOCATION_INVALID",
      ]);
      let phaseRejected = phaseRows.some((row) =>
        row.issues.some((issue) => phaseCodes.has(issue.code)),
      );
      const phaseSignature = new Set(
        phaseRows.map(
          (row) =>
            `${row.phaseName}|${row.startWeeks}|${row.endWeeks}|${row.allocation}`,
        ),
      );
      if (phaseSignature.size > 1) {
        ...
        issues.push({
          ...
          message: "Phase rows disagree on name, timing, or allocation",
        });
      }
      if (
        phaseRejected ||
        !phaseFirst.phaseId ||
        !phaseFirst.phaseName ||
        phaseFirst.startWeeks === null ||
        phaseFirst.endWeeks === null ||
        !phaseFirst.allocation
      ) {
        rejectedPhaseCount += 1;
        rejectedOperationCount += new Set(
          phaseRows.map((row) => row.operationId),
        ).size;
      ...
      const operationGroups = groupBy(
        phaseRows,
        (row) => row.operationId || `row-${row.rowNumber}`,
      );
      ...
      for (const [operationKey, operationRows] of operationGroups) {
        ...
        const operationCodes = new Set([
          ...
          "REQUIREMENT_VALUE_REQUIRED",
          "REQUIREMENT_VALUE_INVALID",
          "REQUIREMENT_BASIS_INVALID",
          "SETUP_REQUIREMENT_STATE_INVALID",
          "SETUP_REQUIREMENT_VALUE_REQUIRED",
          "SETUP_QUANTITY_INVALID",
          "BATCH_SIZE_INVALID",
        ]);
        let operationRejected = operationRows.some((row) =>
          row.issues.some((issue) => operationCodes.has(issue.code)),
        );
        const operationSignature = new Set(
          operationRows.map(
            (row) => `${row.operationName}|${row.sequence}|${row.phaseId}`,
          ),
        );
        if (operationSignature.size > 1) {
          ...
          issues.push({
            ...
            severity: "error",
            code: "OPERATION_INCONSISTENT",
            message: "Operation rows disagree on name, sequence, or phase",
          });
        }
        const resourceIds = operationRows
          .map((row) => row.resourceGroupId)
          .filter(Boolean);
        if (new Set(resourceIds).size !== resourceIds.length) {
          ...
          issues.push({
            ...
            message: "Operation contains duplicate resource requirements",
          });
        }

        if (
          operationRejected ||
          !operationFirst.operationId ||
          !operationFirst.operationName ||
          operationFirst.sequence === null
        ) {
        ...
        for (const row of operationRows) {
          ...
          requirements.push({
            ...
            ...(row.basis && row.basis !== "perUnit"
              ? { basis: row.basis }
              : {}),
            ...(row.setupQuantity !== null
              ? { setupQuantity: row.setupQuantity }
              : {}),
            ...(row.setupState
              ? {
        ...
        if (requirements.length === 0) {
          ...
          issues.push({
            entityKey: operationKey,
            severity: "error",
            code: "OPERATION_REQUIREMENTS_EMPTY",
            message: "Operation has no valid resource requirements",
          });
          continue;
        }
      ...
      if (phaseOperations.length === 0) {
        ...
        issues.push({
          entityKey: phaseKey,
          severity: "error",
          code: "PHASE_OPERATIONS_EMPTY",
          message: "Phase has no valid operations",
        });
        continue;
      }
      phases.push({
    ...
    if (phases.length === 0 || operations.length === 0) {
      issues.push({
        entityKey: revisionKey,
        severity: "error",
        code: "REVISION_CONTENT_EMPTY",
        message: "Revision has no valid phases and operations",
      });
  ...
  const phaseCount = new Set(
    parsedRows.map((row) => row.phaseId || `row-${row.rowNumber}`),
  ).size;
  const operationCount = new Set(
    parsedRows.map((row) => row.operationId || `row-${row.rowNumber}`),
  ).size;
    ...
    controlTotals: {
      ...
      warningRows: new Set(
        issues
          .filter(
            (issue) =>
              issue.severity === "warning" && issue.rowNumber !== undefined,
          )
      ...
      phaseCount,
      acceptedPhaseCount,
      rejectedPhaseCount: Math.max(
        rejectedPhaseCount,
        phaseCount - acceptedPhaseCount,
      ),
      operationCount,
      acceptedOperationCount,
      rejectedOperationCount: Math.max(
        rejectedOperationCount,
        operationCount - acceptedOperationCount,
      ),
      requirementCount,
      acceptedRequirementCount,
      rejectedRequirementCount: Math.max(
        rejectedRequirementCount,
        requirementCount - acceptedRequirementCount,
      ),
    },
  };
}

export function mergeRoutingImport(
  model: CapacityModel,
  records: RoutingRevision[],
): CapacityModel {
  const existingIds = new Set(
    model.routingRevisions.map((revision) => revision.id),
  );
  for (const record of records) {
    if (existingIds.has(record.id))
      throw new Error(`Routing revision already exists: ${record.id}`);
    const overlaps = model.routingRevisions.some(
      (existing) =>
    ...
    if (overlaps)
      throw new Error(
        `Routing revision overlaps an existing revision for product: ${record.productId}`,
      );
  }
  return {
    ...model,
    routingRevisions: [...model.routingRevisions, ...records],
  };
}
