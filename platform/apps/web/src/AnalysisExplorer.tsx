...
function LoadCapacityComboChart({ labels, load, capacity }: { labels: string[]; load: number[]; capacity: number[] }) {
  ...
  const tickIndexes = labels.map((_, index) => index).filter(index => labels.length <= 12 || index % Math.ceil(labels.length / 10) === 0 || index === labels.length - 1);
  return <div className="chart-frame">
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Demand bars and capacity line by period">
      {[0, .25, .5, .75, 1].map(fraction => {
        const value = maximum * fraction;
        return <g key={fraction}><line className="chart-grid" x1={margin.left} x2={width - margin.right} y1={y(value)} y2={y(value)} /><text className="chart-axis-label" x={margin.left - 10} y={y(value) + 4} textAnchor="end">{formatNumber(value)}</text></g>;
      })}
      {load.map((value, index) => <rect key={index} className="timeline-bar" x={x(index) - barWidth / 2} y={y(value)} width={barWidth} height={Math.max(1, margin.top + plotHeight - y(value))}><title>{`${labels[index]} · Demand/load: ${formatNumber(value)}`}</title></rect>)}
      <path d={capacityPath} className="chart-line baseline-capacity" />
      {capacity.map((value, index) => <circle key={index} className="chart-point baseline-capacity" cx={x(index)} cy={y(value)} r="4"><title>{`${labels[index]} · Available capacity: ${formatNumber(value)}`}</title></circle>)}
      {tickIndexes.map(index => <text key={index} className="chart-x-label" x={x(index)} y={height - 22} textAnchor="middle">{labels[index]}</text>)}
    </svg>
    <div className="chart-legend"><span><i className="demand-line" />Demand / load</span><span><i className="baseline-capacity" />Available capacity</span></div>
  </div>;
}
...
function GapBarChart({ labels, baseline, recovery }: { labels: string[]; baseline: number[]; recovery?: number[] }) {
  ...
  return <div className="chart-frame">
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Capacity gap chart">
      ...
      <text className="chart-axis-label" x={margin.left - 10} y={y(-maxAbs) + 4} textAnchor="end">-{formatNumber(maxAbs)}</text>
      {baseline.map((value, index) => bar(value, index, recovery ? -barWidth * .58 : 0, "baseline-gap"))}
      {recovery?.map((value, index) => bar(value, index, barWidth * .58, "recovery-gap"))}
...
function DemandExplorer({ model, scenarioId, resolution }: { model: CapacityModel; scenarioId: string; resolution: ExploreResolution }) {
  ...
  function addDemand() {
    ...
    model.demand.push({ id: `demand-${crypto.randomUUID()}`, scenarioId, productId: targetProduct, shipDate: model.horizonStart, quantity: 0, demandClass: "forecast", sourceSystem: "Assessment Studio edit" });
    forceRender(value => value + 1);
  }
  function removeDemand(id: string) {
    const index = model.demand.findIndex(record => record.id === id);
    if (index >= 0) model.demand.splice(index, 1);
    forceRender(value => value + 1);
  }
  return <>
...
function RiskHeatmap({ model, calculation, resolution, resourceKind, onSelectResource }: {
  model: CapacityModel;
...
export default function AnalysisExplorer({ model, baseline, comparison, onBack, onContinue }: AnalysisExplorerProps) {
  ...
  return <section className="panel explorer-panel">
    <div className="panel-heading"><div><span className="eyebrow blue">Step 5 · Explore</span><h2>Interrogate the capacity result</h2></div><p>Switch resources, time resolution, and analytical views. Demand and lead-time inputs can now be reviewed and edited directly.</p></div>
    ...
    {view === "capacity" ? <>
      <div className="analysis-grid"><div className="analysis-card wide"><div className="analysis-card-heading"><div><h3>Demand/load versus available capacity</h3><p>{selectedGroup?.name} · bars show load; line shows capacity</p></div></div><LoadCapacityComboChart labels={aligned.map(point => point.baseline.label)} load={aligned.map(point => overlay && point.recovery ? point.recovery.load : point.baseline.load)} capacity={aligned.map(point => overlay && point.recovery ? point.recovery.capacity : point.baseline.capacity)} /></div>
