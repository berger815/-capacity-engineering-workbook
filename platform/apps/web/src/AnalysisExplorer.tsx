...
function LineChart({
  ...
  const finite = [...series.flatMap(item => item.values), ...referenceLines.map(item => item.value)]
  ...
  const padding = Math.max((maximum - minimum) * 0.08, maximum === 0 ? 1 : Math.abs(maximum) * 0.04);
    ...
    <div className="chart-frame">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Time-series chart">
        {[0, 0.25, 0.5, 0.75, 1].map(fraction => {
          ...
          const py = y(value);
          return <g key={fraction}><line className="chart-grid" x1={margin.left} x2={width - margin.right} y1={py} y2={py} /><text className="chart-axis-label" x={margin.left - 10} y={py + 4} textAnchor="end">{valueFormatter(value)}</text></g>;
        })}
        {referenceLines.map(reference => <g key={reference.label}><line className={`chart-reference ${reference.className}`} x1={margin.left} x2={width - margin.right} y1={y(reference.value)} y2={y(reference.value)} /><text className="chart-reference-label" x={width - margin.right} y={y(reference.value) - 6} textAnchor="end">{reference.label}</text></g>)}
        {series.map(item => {
          ...
          item.values.forEach((value, index) => {
            ...
            current += `${current ? " " : ""}${x(index)},${y(value)}`;
          });
          if (current) segments.push(current);
          return <g key={item.label}>{segments.map((points, index) => <polyline key={index} className={`chart-line ${item.className}`} points={points} />)}{item.values.map((value, index) => value === null || !Number.isFinite(value) ? null : <circle key={index} className={`chart-point ${item.className}`} cx={x(index)} cy={y(value)} r="4"><title>{`${labels[index]} · ${item.label}: ${valueFormatter(value)}`}</title></circle>)}</g>;
        })}
        {tickIndexes.map(index => <text key={index} className="chart-x-label" x={x(index)} y={height - 22} textAnchor="middle">{labels[index]}</text>)}
      </svg>
      <div className="chart-legend">{series.map(item => <span key={item.label}><i className={item.className} />{item.label}</span>)}</div>
    </div>
  );
}
...
function GapBarChart({ labels, baseline, recovery }: { labels: string[]; baseline: number[]; recovery?: number[] }) {
  ...
  const barWidth = Math.max(5, Math.min(22, groupWidth * (recovery ? 0.28 : 0.55)));
  const tickIndexes = labels.map((_, index) => index).filter(index => labels.length <= 12 || index % Math.ceil(labels.length / 10) === 0 || index === labels.length - 1);

  function bar(value: number, index: number, offset: number, className: string) {
    const zero = y(0);
    const py = y(value);
    const x = margin.left + groupWidth * index + groupWidth / 2 + offset - barWidth / 2;
    return <rect key={`${className}-${index}`} className={`gap-bar ${value < 0 ? "negative-bar" : "positive-bar"} ${className}`} x={x} y={Math.min(zero, py)} width={barWidth} height={Math.max(1, Math.abs(zero - py))}><title>{`${labels[index]} · ${value.toFixed(1)}`}</title></rect>;
  }

  return (
    <div className="chart-frame">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Capacity gap chart">
        <line className="chart-zero" x1={margin.left} x2={width - margin.right} y1={y(0)} y2={y(0)} />
        <text className="chart-axis-label" x={margin.left - 10} y={y(maxAbs) + 4} textAnchor="end">{formatNumber(maxAbs)}</text>
        <text className="chart-axis-label" x={margin.left - 10} y={y(0) + 4} textAnchor="end">0</text>
        <text className="chart-axis-label" x={margin.left - 10} y={y(-maxAbs) + 4} textAnchor="end">-{formatNumber(maxAbs)}</text>
        {baseline.map((value, index) => bar(value, index, recovery ? -barWidth * 0.58 : 0, "baseline-gap"))}
        {recovery?.map((value, index) => bar(value, index, barWidth * 0.58, "recovery-gap"))}
        {tickIndexes.map(index => <text key={index} className="chart-x-label" x={margin.left + groupWidth * index + groupWidth / 2} y={height - 22} textAnchor="middle">{labels[index]}</text>)}
      </svg>
      <div className="chart-legend"><span><i className="baseline-gap" />Baseline gap</span>{recovery ? <span><i className="recovery-gap" />Recovery gap</span> : null}</div>
    </div>
  );
}
...
function DemandExplorer({ model, scenarioId, resolution }: { model: CapacityModel; scenarioId: string; resolution: ExploreResolution }) {
  const [productId, setProductId] = useState("all");
  const records = model.demand.filter(record => record.scenarioId === scenarioId).filter(record => productId === "all" || record.productId === productId);
  const grouped = new Map<string, { label: string; quantity: number }>();
  for (const record of records) {
    const target = demandBucket(record.shipDate, resolution);
    const current = grouped.get(target.key) ?? { label: target.label, quantity: 0 };
  ...
  const productTotals = model.products.map(product => ({
    product,
    quantity: model.demand.filter(record => record.scenarioId === scenarioId && record.productId === product.id).reduce((sum, record) => sum + record.quantity, 0),
  })).filter(item => item.quantity > 0).sort((a, b) => b.quantity - a.quantity);
  const total = productTotals.reduce((sum, item) => sum + item.quantity, 0);

  return <>
    <div className="explorer-subfilters"><label>Product<select value={productId} onChange={event => setProductId(event.target.value)}><option value="all">All products</option>{model.products.map(product => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label></div>
    <div className="analysis-card wide"><div className="analysis-card-heading"><div><h3>Shipment demand profile</h3><p>Demand is shown at ship date. Capacity charts shift applicable work into earlier lead-time periods.</p></div></div><LineChart labels={points.map(point => point.label)} series={[{ label: "Ship quantity", values: points.map(point => point.quantity), className: "demand-line" }]} valueFormatter={value => value.toFixed(0)} /></div>
...
function LeadTimeExplorer({ model }: { model: CapacityModel }) {
  const [productId, setProductId] = useState(model.products[0]?.id ?? "");
  useEffect(() => {
    if (!model.products.some(product => product.id === productId)) setProductId(model.products[0]?.id ?? "");
  }, [model, productId]);
  ...
  const width = 980;
  const rowHeight = 72;
  const height = 76 + phases.length * rowHeight;
  const left = 170;
  const right = 40;
  ...
  return <>
    <div className="explorer-subfilters"><label>Product<select value={productId} onChange={event => setProductId(event.target.value)}>{model.products.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><div className="route-meta"><span>Revision</span><strong>{revision.revision}</strong><span>Effective</span><strong>{revision.effectiveFrom}</strong></div></div>
    <div className="analysis-card wide"><div className="analysis-card-heading"><div><h3>Lead-time route timeline</h3><p>Work is positioned by weeks before ship. Ship date is the zero point on the right.</p></div><strong>{maxWeeks} weeks</strong></div><div className="timeline-scroll"><svg className="route-timeline" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${product.name} lead-time route`}>
...
function RiskHeatmap({
  ...
  const ranked = groups.map(group => {
    const points = aggregateResourceResults(calculation.results, group.id, resolution);
    const maximum = Math.max(-1, ...points.map(point => point.utilization === null ? -1 : Number.isFinite(point.utilization) ? point.utilization : Number.MAX_SAFE_INTEGER));
    return { group, points, maximum };
  }).filter(item => item.points.some(point => point.load > 0)).sort((a, b) => b.maximum - a.maximum).slice(0, 10);
...
export default function AnalysisExplorer({ model, baseline, comparison, onBack, onContinue }: AnalysisExplorerProps) {
  ...
  useEffect(() => {
    if (!groups.some(group => group.id === resourceGroupId)) setResourceGroupId(groups[0]?.id ?? "");
  }, [groups, resourceGroupId]);
  useEffect(() => {
    if (!comparison) setCompareMode("baseline");
  }, [comparison]);
  ...
  const worst = highestUtilization((overlay ? comparison.comparison : baseline).results, resourceGroupId);
  const nativeRows = (overlay ? comparison.comparison : baseline).results.filter(row => row.resourceGroupId === resourceGroupId).sort((a, b) => a.periodStart.localeCompare(b.periodStart));
  const riskRows = [...nativeRows].filter(row => row.load > 0).sort((a, b) => {
  ...
  const recoveryFte = recoveryPoints?.map(point => {
    const baselinePoint = baselinePoints.find(candidate => candidate.key === point.key);
    const baseAvailable = resourceQuantityAt(model, resourceGroupId, point.periodStart);
    const availableFte = baselinePoint && baselinePoint.capacity > 0 ? baseAvailable * point.capacity / baselinePoint.capacity : baseAvailable;
    const requiredFte = point.utilization === null || !Number.isFinite(point.utilization) ? null : availableFte * point.utilization;
    return { ...point, availableFte, requiredFte, fteGap: requiredFte === null ? null : availableFte - requiredFte };
  }) ?? null;
  ...
  return (
    <section className="panel explorer-panel">
      <div className="panel-heading"><div><span className="eyebrow blue">Step 5 · Explore</span><h2>Interrogate the capacity result</h2></div><p>Switch resources, time resolution, and analytical views. The guided workflow remains available, but the result is no longer a black box or a single ranked list.</p></div>
      <div className="explorer-toolbar">
        <label>Resolution<select value={resolution} onChange={event => setResolution(event.target.value as ExploreResolution)}><option value="native">{model.planningGranularity === "week" ? "Weekly" : "Monthly"}</option><option value="quarter">Quarterly</option><option value="year">Annual</option></select></label>
        <label>Resource class<select value={resourceKind} onChange={event => setResourceKind(event.target.value as ResourceKind | "all")}><option value="all">All resource classes</option>{availableKinds.map(kind => <option value={kind} key={kind}>{kind}</option>)}</select></label>
        <label>Resource group<select value={resourceGroupId} onChange={event => setResourceGroupId(event.target.value)}>{groups.map(group => <option value={group.id} key={group.id}>{group.name}</option>)}</select></label>
        <label>Scenario view<select value={compareMode} onChange={event => setCompareMode(event.target.value as CompareMode)}><option value="baseline">Baseline</option>{comparison ? <option value="overlay">Baseline vs recovery</option> : null}</select></label>
      </div>
      <div className="explorer-tabs" role="tablist">{(["capacity", "gaps", "demand", "headcount", "leadtime"] as ExploreView[]).map(item => <button key={item} type="button" className={view === item ? "active" : ""} onClick={() => setView(item)}>{item === "capacity" ? "Load & utilization" : item === "gaps" ? "Gaps & constraints" : item === "demand" ? "Demand & product mix" : item === "headcount" ? "Headcount" : "Lead-time routes"}</button>)}</div>
      ...
      {view === "capacity" ? <>
        <div className="analysis-grid"><div className="analysis-card wide"><div className="analysis-card-heading"><div><h3>Load versus available capacity</h3><p>{selectedGroup?.name} · {selectedGroup?.capacityUnit}</p></div></div><LineChart labels={aligned.map(point => point.baseline.label)} series={[{ label: "Baseline load", values: aligned.map(point => point.baseline.load), className: "baseline-load" }, { label: "Baseline capacity", values: aligned.map(point => point.baseline.capacity), className: "baseline-capacity" }, ...(overlay ? [{ label: "Recovery load", values: aligned.map(point => point.recovery?.load ?? null), className: "recovery-load" }, { label: "Recovery capacity", values: aligned.map(point => point.recovery?.capacity ?? null), className: "recovery-capacity" }] : [])]} /></div>
        <div className="analysis-card wide"><div className="analysis-card-heading"><div><h3>Utilization by period</h3><p>85% is the watch threshold; 100% is the physical capacity line.</p></div></div><LineChart labels={aligned.map(point => point.baseline.label)} series={[{ label: "Baseline utilization", values: aligned.map(point => point.baseline.utilization === null ? null : point.baseline.utilization * 100), className: "baseline-utilization" }, ...(overlay ? [{ label: "Recovery utilization", values: aligned.map(point => point.recovery?.utilization === null || point.recovery?.utilization === undefined ? null : point.recovery.utilization * 100), className: "recovery-utilization" }] : [])]} valueFormatter={value => `${Math.round(value)}%`} referenceLines={[{ value: 85, label: "Watch 85%", className: "watch-line" }, { value: 100, label: "Capacity 100%", className: "capacity-line" }]} /></div></div>
        <RiskHeatmap model={model} calculation={overlay ? comparison.comparison : baseline} resolution={resolution} resourceKind={resourceKind} onSelectResource={setResourceGroupId} />
        <ConstraintExplorer model={model} scenarioId={(overlay ? comparison.comparison : baseline).scenarioId} rows={riskRows} title={`${selectedGroup?.name ?? "Resource"} period detail`} subtitle="Use Explain to reconcile a native period back to products, operations, lead-time phases, setup, and demand records." />
      </> : null}

      {view === "gaps" ? <>
        <div className="analysis-card wide"><div className="analysis-card-heading"><div><h3>Capacity balance by period</h3><p>Positive bars are remaining capacity. Negative bars are gaps that require action.</p></div></div><GapBarChart labels={aligned.map(point => point.baseline.label)} baseline={aligned.map(point => point.baseline.gap)} recovery={overlay ? aligned.map(point => point.recovery?.gap ?? 0) : undefined} /></div>
        <RiskHeatmap model={model} calculation={overlay ? comparison.comparison : baseline} resolution={resolution} resourceKind={resourceKind} onSelectResource={setResourceGroupId} />
        <ConstraintExplorer model={model} scenarioId={(overlay ? comparison.comparison : baseline).scenarioId} rows={riskRows} title="Constraint periods ranked by severity" subtitle="Select Explain for product, routing, standard-hour, setup, and demand lineage." />
      </> : null}

      {view === "demand" ? <DemandExplorer model={model} scenarioId={baseline.demandSourceScenarioId ?? baseline.scenarioId} resolution={resolution} /> : null}

      {view === "headcount" ? selectedGroup?.kind !== "labor" ? <div className="callout amber"><span>Labor resource required</span><strong>Select a labor resource group to translate load and capacity into effective FTE equivalents.</strong></div> : <>
        <div className="analysis-card wide"><div className="analysis-card-heading"><div><h3>Required versus effective available FTE</h3><p>This converts the selected labor group’s utilization into FTE equivalents; it is not payroll headcount.</p></div></div><LineChart labels={baselineFte.map(point => point.label)} series={[{ label: "Baseline required FTE", values: baselineFte.map(point => point.requiredFte), className: "required-fte" }, { label: "Baseline available FTE", values: baselineFte.map(point => point.availableFte), className: "available-fte" }, ...(overlay && recoveryFte ? [{ label: "Recovery required FTE", values: recoveryFte.map(point => point.requiredFte), className: "recovery-required-fte" }, { label: "Recovery available FTE", values: recoveryFte.map(point => point.availableFte), className: "recovery-available-fte" }] : [])]} valueFormatter={value => value.toFixed(1)} /></div>
        <div className="table-card"><div className="card-title-row"><div><h3>Headcount-equivalent detail</h3><small>Negative FTE gap means the modeled period requires more productive labor capacity.</small></div></div><div className="table-wrap"><table><thead><tr><th>Period</th><th className="number">Baseline required</th><th className="number">Baseline available</th><th className="number">Baseline gap</th>{overlay ? <><th className="number">Recovery required</th><th className="number">Recovery available</th><th className="number">Recovery gap</th></> : null}</tr></thead><tbody>{baselineFte.map((point, index) => <tr key={point.key}><td>{point.label}</td><td className="number">{point.requiredFte?.toFixed(1) ?? "—"}</td><td className="number">{point.availableFte.toFixed(1)}</td><td className={`number ${(point.fteGap ?? 0) < 0 ? "negative" : ""}`}>{point.fteGap?.toFixed(1) ?? "—"}</td>{overlay && recoveryFte ? <><td className="number">{recoveryFte[index]?.requiredFte?.toFixed(1) ?? "—"}</td><td className="number">{recoveryFte[index]?.availableFte.toFixed(1) ?? "—"}</td><td className={`number ${(recoveryFte[index]?.fteGap ?? 0) < 0 ? "negative" : ""}`}>{recoveryFte[index]?.fteGap?.toFixed(1) ?? "—"}</td></> : null}</tr>)}</tbody></table></div></div>
      </> : null}

      {view === "leadtime" ? <LeadTimeExplorer model={model} /> : null}

      <div className="panel-actions split"><button className="secondary" type="button" onClick={onBack}>Back to calculation</button><button className="primary" type="button" onClick={onContinue}>Continue to recovery</button></div>
    </section>
  );
}
