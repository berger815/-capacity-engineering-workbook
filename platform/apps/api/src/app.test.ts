...
describe("Capacity Assurance API", () => {
  ...
  it("previews and transactionally applies a product import", () => {
    ...
    const preview = routeApiRequest("POST", "/v1/import/products/preview", { model: northstarRecoveryModel, csv, mapping: genericProductProfile.mapping, mode: "replaceById" });
    expect(preview.statusCode).toBe(200);
    expect(preview.body).toMatchObject({ controlTotals: { acceptedRows: 1, addedRecords: 1, rejectedRows: 0 } });
    const applied = routeApiRequest("POST", "/v1/import/products/apply", { model: northstarRecoveryModel, csv, mapping: genericProductProfile.mapping, mode: "replaceById" });
  ...
  it("blocks an invalid resource-group dependency without mutating the model", () => {
    ...
    const response = routeApiRequest("POST", "/v1/import/resource-groups/apply", { model: northstarRecoveryModel, csv, mapping: genericResourceGroupProfile.mapping });
  ...
  it("applies the valid sibling operation from a partially rejected routing revision only when accepted", () => {
    ...
    expect(routeApiRequest("POST", "/v1/import/routing/apply", { model: northstarRecoveryModel, csv, mapping: genericRoutingProfile.mapping }).statusCode).toBe(422);
    const applied = routeApiRequest("POST", "/v1/import/routing/apply", { model: northstarRecoveryModel, csv, mapping: genericRoutingProfile.mapping, acceptPartial: true });
    expect(applied.statusCode).toBe(200);
    ...
    expect(body.model.routingRevisions.find(item => item.revision === "Z")?.operations).toHaveLength(1);
    expect(body.import.controlTotals.rejectedOperationCount).toBe(1);
  });

  it("previews mapped demand with reconciliation totals", () => {
    const csv = ["product_id,ship_date,quantity,class,record_id", "hx100,2027-10-15,10,forecast,R1", "hx200,2027-11-15,5,firm,R2"].join("\n");
    const response = routeApiRequest("POST", "/v1/import/demand/preview", { model: northstarRecoveryModel, scenarioId: "baseline", csv, mapping: demandMapping });
    expect(response.statusCode).toBe(200);
  ...
  it("blocks partial demand replacement unless explicitly accepted", () => {
    const csv = ["product_id,ship_date,quantity,class,record_id", "hx100,2027-10-15,10,forecast,R1", "unknown,2027-11-15,5,firm,R2"].join("\n");
    expect(routeApiRequest("POST", "/v1/import/demand/apply", { model: northstarRecoveryModel, scenarioId: "baseline", csv, mapping: demandMapping }).statusCode).toBe(422);
    const accepted = routeApiRequest("POST", "/v1/import/demand/apply", { model: northstarRecoveryModel, scenarioId: "baseline", csv, mapping: demandMapping, acceptPartial: true });
  ...
  it("calculates Northstar and pulls long-lead work into 2026", () => {
    ...
    expect(response.statusCode).toBe(200);
    const result = response.body as { results: Array<{ periodStart: string; load: number; resourceGroupId: string }>; governingConstraint: unknown };
  ...
  it("compares an immutable baseline with its governed recovery scenario", () => {
    const response = routeApiRequest("POST", "/v1/compare", { model: northstarRecoveryModel, baselineScenarioId: "baseline", comparisonScenarioId: "recovery-1" });
    expect(response.statusCode).toBe(200);
  ...
  it("generates standalone HTML and portable JSON reports", () => {
    const html = routeApiRequest("POST", "/v1/report/decision", { model: northstarRecoveryModel, baselineScenarioId: "baseline", comparisonScenarioId: "recovery-1", format: "html" });
    expect(html.statusCode).toBe(200);
    expect((html.body as { content: string }).content).toContain("Capacity Assurance Decision Package");
    const json = routeApiRequest("POST", "/v1/report/decision", { model: northstarRecoveryModel, baselineScenarioId: "baseline", comparisonScenarioId: "recovery-1", format: "json" });
